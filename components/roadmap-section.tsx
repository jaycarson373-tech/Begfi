            ...
            {phases.map((phase, index) => (
              <motion.article key={phase.phase} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.55, delay: index * 0.08 }} className="premium-card relative min-h-[420px] p-6 sm:p-8">
                <div className="relative z-10 mb-10 flex items-center justify-between gap-3">
                  <span className="grid h-10 w-10 place-items-center rounded-full border border-[#6b9cff]/[0.35] bg-[#07142e] text-sm font-black text-white shadow-[0_0_24px_rgba(30,94,255,0.3)]">{index + 1}</span>
                  <span className={`rounded-full border px-3 py-1.5 text-[0.65rem] font-extrabold uppercase ${phase.status === "Live" ? "border-[#4f8cff]/50 bg-[#1e5eff]/[0.15] text-[#a8c4ff]" : "border-white/10 bg-white/[0.04] text-white/[0.45]"}`}>{phase.status}</span>
                </div>
                ...
                <ul className="mt-7 grid gap-3 text-sm text-white/[0.55]">
                  {phase.points.map((point) => <li key={point} className="flex gap-3"><span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[#6b9cff] shadow-[0_0_8px_rgba(59,130,246,0.8)]" />{point}</li>)}
                </ul>
